...
public class Weapon extends Item{
    ...
    public Weapon(String name, int price, int worth, Player player, int tier){
        ...
        baseDamage = (int)Math.floor(Math.random()*((player.getInfluence()+11)- staminaCost+3+tier)+staminaCost+2+tier);
    ...
    public Weapon(String name, int price, int worth, int staminaCost, Player player, int ammoMax, int baseDamage){
        super(name, "weapon", price, worth, player);
        this.tier = 0;
        this.price = price;
        this.worth = worth;
